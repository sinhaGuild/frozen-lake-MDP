...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
    ...
    while True:
        ...
        for state in range(nS):
            ...
            # Compute the expected value of the current state. gamma=0.9
            expected_value = sum(
                [
            ...
            # Compute the difference between the old value function and the new value function for current state
            difference = abs(value_function[state] - expected_value)

            # Update the value function for the current state
            value_function[state] = expected_value

            # Update the maximum difference between the old value function and the new value function over all states
            delta = max(delta, difference)

        # Check if the difference between the old value function and the new value function is less than the tolerance, tol=1e-3
        if delta < tol:
            break
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
    ...
    for state in range(nS):
        # Initialize the best action and best Q-value to None and negative infinity, respectively
        best_action, best_q_value = None, float("-inf")

        # Iterate over all actions a
        for action in range(nA):
            # Compute the Q-value of the current action for the current state
            q_value = sum(
                [
            ...
            # Update the best action and best Q-value if the current Q-value is greater than the best Q-value
            if q_value > best_q_value:
                best_action, best_q_value = action, q_value

        # Update the new policy for the current state
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
    ...
    while True:
        ...
        # Iterate over all states s
        for state in range(nS):
            # Initialize the old value function to the current value function for the current state
            ...
            for action in range(nA):
                ...
                q_value = sum(
                    [
                        probability * (reward + gamma * value_function[next_state])
                        for (probability, next_state, reward, terminal) in P[state][
                            action
                        ]
                    ]
                )

                # Update the best Q-value if the current Q-value is greater than the best Q-value
                best_q_value = max(best_q_value, q_value)

            # Update the value function for the current state
            value_function[state] = best_q_value

            # Update delta with the absolute difference between the old value function and the new value function for the current state
            delta = max(delta, abs(old_value_function - value_function[state]))

        # Check if delta is less than the tolerance
        if delta < tol:
            break
